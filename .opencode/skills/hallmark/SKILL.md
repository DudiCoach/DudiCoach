...
- **Source of truth:** `docs/engineering-policy.md` — §UI/UX Reviewer responsibilities (G4).
- **Use for:** building or redesigning DudiCoach UI. The app is a Polish-language, dark-only theme, mobile-first athlete panel + desktop coach panel.
- **Key DudiCoach UI constraints:**
  - All user-facing strings from `lib/i18n/pl.ts` — no hardcoded Polish in components
  - Dark theme via design tokens in `app/globals.css` — never hardcode hex values
  - Auto-save pattern (no Save buttons), debounced optimistic updates
  - shadcn/ui + Radix primitives in `components/ui/`
  - DM Sans font via Next.js font infrastructure
- **Related agents:** `.claude/agents/ui-reviewer.md` (G4 gate), `.claude/agents/developer-frontend.md`.

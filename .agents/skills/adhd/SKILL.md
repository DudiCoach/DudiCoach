...
This skill is prompt-only. It reads and writes no files and runs no shell
commands, so it behaves identically on Windows, macOS, and Linux.

Host mapping on Windows:

- **OpenCode** — the skill is discovered from `.agents/skills/adhd/SKILL.md`
  in the repository and loaded through the `skill` tool. The Phase 1 fan-out
  uses OpenCode's `task` tool for the isolated branches.
- **Codex** — discovered from the same `.agents/skills/` path. Invoke with
  `$adhd "your problem"`. The fan-out uses Codex subagents.

Windows notes:

1. **Isolation invariant still applies.** If the host cannot spawn parallel
   subagents, the branches degrade to sequential calls in one context. That
   is a wider single thought, not ADHD. Say so in the output rather than
   presenting sequential results as divergent ones.
2. **No shell dependency.** Do not translate any part of this skill into
   PowerShell or `cmd` commands. There is nothing to run.
3. **Optional CLI.** The companion CLI is not required and is not vendored
   here. If a user wants it, install it in PowerShell with:
   ...
   The CLI needs `ANTHROPIC_API_KEY` in the environment. Set it in
   PowerShell with `$env:ANTHROPIC_API_KEY = "..."` for the current session.
4. **Description length.** The frontmatter description is intentionally a
   single line under 600 characters because some Codex builds truncate or
   reject multi-line YAML block-scalar descriptions. Keep it that way if you
   edit this file.

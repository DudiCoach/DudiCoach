...
- **Use for:** creating or editing skills in `.opencode/skills/` for this repo.
- **Skill location:** repo-local skills live in `.opencode/skills/<name>/SKILL.md` and are loaded via `opencode.json` → `skills.paths`.
- **Related:** `.claude/agents/` (Claude agents) and `.codex/agents/` (Codex TOML agents) are the agent definitions; skills complement them with reusable technique guidance.

...
Oxlint ships as a cross-platform native binary through npm, so Windows needs no
extra toolchain. The installer is plain Node and imports only `node:fs`,
`node:path`, and `node:url`.

Use backslash paths from the repository root:

```powershell
node .agents\skills\install-anti-slop\scripts\install.mjs
```

Pass a different destination as the first argument when the repository has an
established tooling layout:

```powershell
node .agents\skills\install-anti-slop\scripts\install.mjs tools\lint\anti-slop
```

The script refuses to overwrite an existing destination. Route an existing copy
through the update procedure in `references/update.md` rather than passing
`--force`.

Install dependencies with npm. Pin both packages to exactly the same version:

```powershell
npm view oxlint version
npm view @oxlint/plugins version
npm install --save-dev --save-exact oxlint@<version> @oxlint/plugins@<version>
```

The lint command is the same on every platform:

```powershell
npm run lint:oxlint
```

Do not translate the JSON configuration into PowerShell. `.oxlintrc.json` is
edited as a file, not generated by a shell command.

## This repository's installation state

anti-slop is already installed here. Record of the install:
`tools/oxlint/anti-slop/UPSTREAM.md`.

Deviations from the defaults above, all deliberate:

1. **Advisory only.** Rules are registered at `"error"` in `.oxlintrc.json`, but
   Oxlint runs through `npm run lint:oxlint` only. It is **not** wired into
   `.github/workflows/ci.yml`. ESLint (`npm run lint`) remains the enforced
   linter. Do not add Oxlint to CI without explicit approval — it would fail
   builds on a large existing baseline.
2. **Extra ignore patterns** for this repository's build output: `.next/**`,
   `out/**`, `build/**`, `coverage/**`, `playwright-report/**`,
   `test-results/**`, `supabase/.temp/**`, `functions/**`, `next-env.d.ts`.
   `functions/**` is Firebase Hosting build output with no tracked source.
3. **Effect rules disabled.** `effect` is not a direct dependency of this
   repository, so the opt-in `anti-slop-effect` plugin stays off. Do not enable
   it because `effect` appears transitively in a lockfile.
4. **No cleanup applied.** The first run reported 1828 findings in owned source.
   They are recorded in `UPSTREAM.md` as a triage baseline. No rule was
   suppressed, no severity weakened, and no unsafe cast added to make the linter
   pass.

When updating here, preserve all four deviations and refresh the baseline table
in `UPSTREAM.md`.

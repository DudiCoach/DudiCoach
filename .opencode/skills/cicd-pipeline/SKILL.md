...
- **Source of truth:** `docs/engineering-policy.md` — §Release readiness check, §Stage gates (G9).
- **Existing CI:** `.github/workflows/ci.yml` — jobs: lint, typecheck, test (Vitest), build, deploy (Firebase hosting + functions + firestore), preview (Firebase preview channels).
- **Deployment:** Firebase Hosting + Cloud Functions (`firebase.json`, `build.sh`), NOT Vercel (older docs may reference Vercel — the repo migrated to Firebase).
- **Secrets:** `FIREBASE_SERVICE_ACCOUNT_DUDICOACH_APP` in GitHub Actions secrets; `.env.example` documents required env vars.

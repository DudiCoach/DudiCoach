...
The checker, linter, and dispatch recorder are zero-dependency Node scripts.
They run on Windows with Node 16 or newer and need no package manager install.
Three things differ from Unix and must be handled explicitly.

### 1. Paths

`<skill-dir>` on Windows is the vendored directory in this repository:

```
.agents\skills\unlazy
```

Use backslashes in PowerShell and `cmd`, or forward slashes in PowerShell
(both work). Absolute form:
...
Approval records live under the user profile, not `~`:

```
%USERPROFILE%\.unlazy\approved
```

In PowerShell that is `$env:USERPROFILE\.unlazy\approved`. To relocate the
store, set `UNLAZY_APPROVAL_DIR` to an owner-private directory outside the
repository:

```powershell
$env:UNLAZY_APPROVAL_DIR = "$env:USERPROFILE\.unlazy-approved"
```

Scoped pipeline state uses backslash paths as well:
`.unlazy\<scope>\PLAN.md`, `.unlazy\<scope>\gates\leaf-*.md`.

### 2. Shell selection

The checker resolves its shell in this order: `--shell`, then `UNLAZY_SHELL`,
then the Node platform default. On Windows the default is `process.env.ComSpec`,
which is `cmd.exe` — **not** PowerShell.

This matters for gate authoring. A `CHECK:` line written with Unix syntax
(`&&`, `$(...)`, `|`, `grep`) fails under `cmd.exe`. Pick one shell per ledger
and write every `CHECK:` line for it:

```powershell
# Force PowerShell for this run
node .agents\skills\unlazy\scripts\gate-check.mjs --shell pwsh GATES.md

# Or pin it for the session
$env:UNLAZY_SHELL = "pwsh"
```

If the project targets `cmd.exe` instead, write `CHECK:` lines as `cmd`
commands. Do not mix shells inside one ledger: a shell or PATH mismatch is a
failed verification, not evidence.

Do not assume `grep`, `tail`, `tr`, `sed`, or `awk` exist. They are absent on
stock Windows. `--shell` changes the interpreter only; it installs nothing.
Portable gates call repository-owned Node scripts:

```markdown
- [ ] G1: schema parses and types check
  CHECK: node scripts/verify-schema.mjs
  EXPECT: schema verification passed
  EVIDENCE: pending
```

Parent re-verification must use the same declared shell and toolchain:

```powershell
node .agents\skills\unlazy\scripts\gate-check.mjs --shell pwsh --reverify GATES.md
```

### 3. The Stop hook does not apply here

`scripts/install-hooks.mjs` and `scripts/stop-hook.mjs` implement a
**Claude Code** Stop hook that writes `.claude/settings.local.json`. OpenCode
and Codex do not support hooks, so on those hosts:

- **Do not run `install-hooks.mjs`.** It has no effect and writes a settings
  file that nothing reads.
- Rely on the three layers that do work everywhere: leaf self-check,
  parent `--reverify`, and branch integration gates.
- Treat the final claim audit as the structural backstop instead of the hook.

The hook scripts are vendored for completeness and for contributors who also
run Claude Code. They are inert on OpenCode and Codex.
...
# Read every CHECK command, then approve
...
# Re-run every runnable gate after a leaf returns
node .agents\skills\unlazy\scripts\gate-check.mjs --reverify GATES.md

# Scoped pipeline status
node .agents\skills\unlazy\scripts\gate-check.mjs --scope api
```

Keep `.unlazy/` and `.unlazy-hook-state.json` out of version control. Do not
commit approval records; they bind absolute paths and full inherited `PATH`,
so they are machine-specific and can leak local directory names.
